...
struct MethodTableWriteableData
{
    ...
    enum
    {
        ...
        enum_flag_NGEN_IsLayoutFixedComputed= 0x00010000, // Set if we have cached the result of IsLayoutFixed computation
        enum_flag_NGEN_IsLayoutFixed        = 0x00020000, // The result of the IsLayoutFixed computation
#endif
        ...
        enum_flag_DynamicCastPerformedOnType        = 0x00800000, // Has the processing for a dynamic cast to this type completed?
    };
...
public:
    ...
    BOOL HasDerivedType() const;
    BOOL HasDerivedTypeInOtherLoaderAllocator() const;
    BOOL HasMultipleDerivedTypes() const;
    BOOL HasDynamicCastToTypeBeenPerformed() const;
    
    inline void SetHasDynamicCastToTypeBeenPerformed()
    {
        CONTRACTL
        {
            THROWS;
        ...
        FastInterlockOr(EnsureWritablePages((ULONG *) &m_dwFlags), enum_flag_DynamicCastPerformedOnType);
    }

    void SetHasMultipleDerivedTypes()
    {
        LIMITED_METHOD_CONTRACT;

        // Used only during method table initialization - no need for logging or Interlocked Exchange.
        m_dwFlags |= enum_flag_DerivedType | enum_flag_MultipleDerivedTypes;
    }

    void DeclareDerivedType(MethodTable *thisType, MethodTable *otherType);
    void NotifyInterfaceUsedViaDynamicCastBehavior(MethodTable *thisType);
private:
    void NotifyInterfaceUsedViaDynamicCastBehavior_Worker(MethodTable *thisType);
public:
...
public:
    ...
    void NotifyInterfaceUsedViaDynamicCastBehavior()
    {
        CONTRACTL {
            THROWS;
            GC_TRIGGERS;
        ...
        return GetWriteableDataForWrite()->NotifyInterfaceUsedViaDynamicCastBehavior(this);
    }
    ...
    inline BOOL HasDynamicCastToTypeBeenPerformed() const
    {
        LIMITED_METHOD_CONTRACT;
        return GetWriteableData()->HasDynamicCastToTypeBeenPerformed();
    }
