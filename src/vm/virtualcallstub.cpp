...
MethodTable *TokenToDevirtualizableMethodTable(DispatchToken token)
{
    if (token.IsTypedToken())
    {
        ...
        if (pMTToken->IsInterface() && !pMTToken->IsComImport() && !pMTToken->IsProjectedFromWinRT() && !pMTToken->IsWinRTRedirectedInterface(TypeHandle::Interop_ManagedToNative)) 
        {
            // Only keep track of interfaces which are known to be devirtualizable. Interfaces associated with COM or WinRT cannot safely be devirtualized
            return pMTToken;
        }
        else
            return NULL;
...
public:
    ...
    bool operator()()
    {
        ...
        {
            _ASSERTE(m_pInterfaceMTToDispatchTo->HasDerivedType());
            if (!m_pInterfaceMTToDispatchTo->HasMultipleDerivedTypes())
            {
                ...
                if (stub == CALL_STUB_EMPTY_ENTRY)
                {
                    ...
                    if (pResolveHolder != NULL && stubKind == SK_LOOKUP)
                    {
                        ...
                        if (pInterfaceMTToDispatchTo != NULL)
                        {
                            ...
                            if (!pInterfaceMTToDispatchTo->HasMultipleDerivedTypes())
                            {
                                stub = target;
                            }
                        }
    ...
    BOOL          throwOnConflict)
{
    ...
    *pInterfaceMT = TokenToDevirtualizableMethodTable(token);
    ...
    {
        _ASSERTE(*pInterfaceMT == NULL); // This can only safely discover ComImport interfaces, or the interface devirt logic is incorrect
    ...
    else if (pMT->IsICastable() && protectedObj != NULL && *protectedObj != NULL && (*pInterfaceMT == NULL))
    {
        // ICastable is only permitted for use with ComImport interfaces (That's the *pInterfaceMT == NULL check above)
